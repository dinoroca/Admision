...
@Component({
  ...
  styleUrls: ['./edit-lenguaje.component.css']
})
...
export class EditLenguajeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._route.params.subscribe(
      ...
      params => {
        ...
        this._preguntaService.obtener_pregunta_lenguaje(this.id, this.token).subscribe(
          response => {
  ...
  actualizar(actualizarForm: any) {
    ...
    if (actualizarForm.valid) {
      if (this.valid) {
        this.load_btn = true;
        this._preguntaService.actualizar_pregunta_lenguaje(this.id, this.pregunta, this.token).subscribe(
          response => {
            if (response.data == undefined) {
              ...
              this._router.navigate(['/admin/lenguaje']);
            }
